...
Todos los valores son números enteros. Calorías en kcal, macros en gramos por ración.`,
  ...
  async function handleSave() {
    ...
    await onSave(recipe.id, updated);
    setSaving(false);
    onClose();
  }
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{maxWidth:720}} onClick={e=>e.stopPropagation()}>
        <div className="modal-header">
          <div className="modal-title">Editar receta</div>
          <button className="modal-close" onClick={onClose}>×</button>
        </div>
        <div className="modal-body">
          ...
          <div style={{background:"var(--paper)",borderRadius:10,padding:"14px 16px",marginBottom:14}}>
            ...
            {aiMsg&&<div className="success-badge" style={{marginBottom:10,display:"flex"}}>{aiMsg}</div>}
      ...
      for (let i=0;i<names.length;i++) {
        try {
          ...
          await onAdd({
            ...
            carbs: parseInt(parsed.carbs)||0, fat: parseInt(parsed.fat)||0,
            source:"PDF",
      ...
      {(inputMode==="manual"||inputMode==="url")&&(
        <div className="form-section">
          ...
          <div style={{background:"var(--paper)",borderRadius:10,padding:"14px 16px",marginBottom:14}}>
            ...
            {aiMsg&&inputMode==="manual"&&<div className="success-badge" style={{marginBottom:10,display:"flex"}}>{aiMsg}</div>}
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
              {[["calories","Calorías"],["protein","Proteínas (g)"],["carbs","Carbohidratos (g)"],["fat","Grasas (g)"]].map(([k,l])=>(
                <div key={k} className="form-group" style={{marginBottom:0}}>
                  <label className="form-label">{l}</label>
          ...
          <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
            {inputMode==="manual"&&(
              <button className="btn-ai" onClick={()=>form.name&&handleSingleAI(`Genera receta completa para "${form.name}"${form.origin?` de ${form.origin}`:""}`)} disabled={loading||!form.name}>
                {loading?<><div className="spinner"/>Procesando...</>:<>✨ Completar todo con IA</>}
              </button>
            )}
            ...
            <div className="content">
              ...
              {loading?(
                ...
                <div className="recipe-grid">
                  {filtered.map(r=><RecipeCard key={r.id} recipe={r} onClick={setSelected} onDelete={handleDelete} onEdit={openEdit}/>)}
                </div>
              )}
            </div>
          </>
